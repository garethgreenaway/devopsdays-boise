{% set salt_projects_root = "/devops/projects2" %}
...
# Temporary fix since git.latest doesn't do the git reset
git-reset:
  cmd.run:
    - name: git reset --hard
...
salt-repo-{{ project }}-{{ branch }}:
  git.latest:
    ...
    - branch: {{ branch }}
    - user: root
    - identity: salt://common/keys/crosconjenkins_id_rsa2
    - force_checkout: True
    - force_clone: True
    - force_fetch: True
    - force_reset: True
...
restart-saltmaster:
  cmd.wait:
    - name: /usr/bin/docker exec salt-master01 supervisorctl restart salt-master
    - watch:
      - file: /etc/salt/master.d/roots.conf

restart-salt-syndic:
  cmd.wait:
    - name: /usr/bin/docker exec salt-master01 supervisorctl restart salt-syndic
    - watch:
      - file: /etc/salt/master.d/roots.conf
